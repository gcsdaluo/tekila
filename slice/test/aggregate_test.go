...
func TestMax(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			res := slice.Max[Integer](tc.input)
...
func TestMin(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			res := slice.Min[Integer](tc.input)
			assert.Equal(t, tc.want, res)
		})
	}
...
func TestSum(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			res := slice.Sum[Integer](tc.input)
			assert.Equal(t, tc.want, res)
		})
	}
...
func testMaxTypes[T tekila.RealNumber](t *testing.T) {
	res := slice.Max[T]([]T{1, 2, 3})
	assert.Equal(t, T(3), res)
}

// testMinTypes 只是用来测试一下满足 Min 方法约束的所有类型
func testMinTypes[T tekila.RealNumber](t *testing.T) {
	res := slice.Min[T]([]T{1, 2, 3})
	assert.Equal(t, T(1), res)
}

// testSumTypes 只是用来测试一下满足 Sum 方法约束的所有类型
func testSumTypes[T tekila.RealNumber](t *testing.T) {
	res := slice.Sum[T]([]T{1, 2, 3})
